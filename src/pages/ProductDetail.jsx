...
import { ArrowLeft, ShieldCheck, ShoppingCart, Zap, Star, Truck, CheckCircle2, AlertTriangle, ChevronLeft, ChevronRight, MapPin, Store, Heart, Leaf } from 'lucide-react'
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border bg-white p-5" style={{ borderColor: '#D5D9D9' }}>
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <p className="text-xs font-bold uppercase tracking-wide" style={{ color: '#007185' }}>
                {product.category}
              </p>
              {product.localHandoff && (
                <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full"
                  style={{ backgroundColor: '#e6f4ea', color: '#067D62' }}>
                  <MapPin size={10} />
                  Local handoff · {product.location || 'Campus'}
                </span>
              )}
            </div>
          ...
          {/* Local handoff — the three-way win */}
          {product.localHandoff && (
            <div className="rounded-lg border p-5" style={{ borderColor: '#067D62', backgroundColor: '#f0faf5' }}>
              <div className="flex items-center gap-2 mb-1">
                <MapPin size={16} style={{ color: '#067D62' }} />
                <h3 className="font-bold text-[#0F1111]">Local handoff · {product.location || 'Campus'}</h3>
              </div>
              <p className="text-sm text-[#565959] mb-4">
                This item is near you. Pick it up hand-to-hand on campus — no shipping, no waiting,
                no return miles. Same Amazon condition guarantee.
              </p>
              <div className="space-y-3">
                <div className="flex items-start gap-2.5">
                  <Store size={15} className="flex-shrink-0 mt-0.5" style={{ color: '#067D62' }} />
                  <p className="text-sm text-[#565959]">
                    <span className="font-semibold text-[#0F1111]">Seller:</span> faster, higher recovery —
                    no reverse-logistics deduction.
                  </p>
                </div>
                <div className="flex items-start gap-2.5">
                  <Heart size={15} className="flex-shrink-0 mt-0.5" style={{ color: '#067D62' }} />
                  <p className="text-sm text-[#565959]">
                    <span className="font-semibold text-[#0F1111]">You:</span> the item at a discount,
                    with an AI-verified condition report and Amazon's buyer guarantee.
                  </p>
                </div>
                <div className="flex items-start gap-2.5">
                  <Leaf size={15} className="flex-shrink-0 mt-0.5" style={{ color: '#067D62' }} />
                  <p className="text-sm text-[#565959]">
                    <span className="font-semibold text-[#0F1111]">Amazon:</span> zero reverse-logistics cost,
                    near-zero carbon on locally-fulfilled items.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-lg border p-4" style={{ borderColor: '#D5D9D9', backgroundColor: '#f0faf5' }}>
            ...
            <p className="text-xs text-[#565959] leading-relaxed">
              Condition and observations were graded by Amazon Bedrock from the product photos.
              The price is calculated by Encore's value-vs-cost engine — not a seller estimate.
            </p>
          </div>
        </div>
      </div>
